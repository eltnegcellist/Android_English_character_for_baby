    ...
    Scaffold(containerColor = MaterialTheme.colorScheme.background) { innerPadding ->
        Column(
            ...
            Card(modifier = Modifier.fillMaxWidth()) {
                Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        ...
                        Text(
                            when (engineMode) {
                                ConversationEngineMode.LITE ->
                                    "Moonshine 日本語Tinyで聞き取り、LiteResponseEngineが返答を選び、Kitten TTS Nano / Kikiで話します。"
                                ConversationEngineMode.FULL ->
                                    "Moonshineの文字起こしと元音声をGemmaが受け取り、会話に合わせて英語を考え、Kitten TTS Nano / Kikiで話します。"
                            },
                        ...
                        when (engineMode) {
                            ...
                            ConversationEngineMode.FULL -> {
                                if (!modelPresent) {
                                    ...
                                    Text(
                                        "Gemmaは導入済みです。FullではLiteと同じMoonshineとKikiを使います。",
                                        style = MaterialTheme.typography.bodySmall,
                                        color = MaterialTheme.colorScheme.onSurfaceVariant,
                                    )
                                }
                    ...
                    if (kittenInstalled) {
                        ...
                        Text(
                            "このエディションの準備時にKikiの音声モデルも導入されます。",
                            style = MaterialTheme.typography.bodySmall,
                            color = MaterialTheme.colorScheme.onSurfaceVariant,
                        )
                    }
            ...
            Text(
                when (engineMode) {
                    ConversationEngineMode.LITE ->
                        "Liteは約64MBの音声スタックで軽く動き、LiteResponseEngineが返答を選びます。"
                    ConversationEngineMode.FULL ->
                        "Fullは同じMoonshineとKikiを使いながら、文字起こしと元音声、直前の会話をGemmaに渡して、その場で英語を生成します。"
                },
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )
