...
private fun ProductionEmmaApp() {
    ...
    val kitten = remember {    val kitten = remember {
        KittenSpeaker(
            context = context,
    ...
    fun loadModel() {
        ...
        if (!requiredModelPresent) {
            ...
            statusMessage = when (engineMode) {
                ConversationEngineMode.LITE ->
                    "Emma LiteのMoonshineとKitten TTSを先に準備してください。"
                ConversationEngineMode.FULL ->
                    "Emma FullのMoonshine・Kitten TTS・Gemmaを先に準備してください。"
            }
            settingsOpen = true
            return
        }
        ...
        EmmaWorkQueue.execute {
            val result = when (requestedMode) {
                ...
                ConversationEngineMode.FULL -> runCatching {
    ...
    fun activateEngineMode(selected: ConversationEngineMode) {
        ...
        statusMessage = when (selected) {
            ...
            ConversationEngineMode.FULL ->
                if (modelPresent) "Fullを選びました。起動します…" else "Fullを選びました。Moonshine・Kiki・Gemmaを準備してください。"
        }
    ...
    val modelPicker = rememberLauncherForActivityResult(    val modelPicker = rememberLauncherForActivityResult(
    ...
    fun startFirstRunSetup(selectedMode: ConversationEngineMode) {
        ...
        EmmaWorkQueue.execute {
            runCatching {
                ...
                mainHandler.post {
                    if (!disposed) {
                        firstRunPhase = "Emma ${selectedMode.label}を起動しています…"
        ...
        EmmaWorkQueue.execute {
            runCatching {
                ...
                if (!MoonshineModelStore.isInstalled(context)) {
                    ...
                    MoonshineModelStore.downloadAndInstall(context) { percent ->
                        mainHandler.post {
                            if (!disposed) fullSetupProgressPercent = percent
                        }
                    }.getOrThrow()
                }
    ...
    fun askEmma(automatic: Boolean = false) {
        ...
        val activeReady = when (engineMode) {
            ...
            ConversationEngineMode.FULL -> lite.isReady() && gemma.isReady()
        }
        ...
        EmmaWorkQueue.execute {
            val result = when (requestedMode) {
                ...
                ConversationEngineMode.FULL -> {
                    val moonshineTranscript = lite.transcribe(wav).getOrNull()
                    if (moonshineTranscript != null) {
                        mainHandler.post {
                            if (!disposed && session.accepts(ticket)) {
                                latestTranscript = moonshineTranscript
                                statusMessage = "音声と文字起こしを使って返事を考えています…"
                            }
                        }
                    }
                    gemma.createEnglishIsland(
                        wavAudio = wav,
                        level = requestedLevel,
                        transcriptHint = moonshineTranscript,
                    ) { transcript ->
                        mainHandler.post {
                            if (!disposed && session.accepts(ticket)) {
